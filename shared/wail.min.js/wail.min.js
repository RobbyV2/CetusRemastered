...
const SECTION_ORDER = [1, 2, 3, 4, 5, 13, 6, 7, 8, 9, 12, 10, 11];
const KIND_FUNC = 0;
const KIND_TABLE = 1;
const KIND_MEMORY = 2;
const KIND_GLOBAL = 3;
...
const OP_RETURN_CALL = 18;
const OP_RETURN_CALL_INDIRECT = 19;
const OP_SELECT_T = 28;
...
const OP_THROW = 8;
const OP_RETHROW = 9;
const OP_THROW_REF = 10;
const OP_CALL_REF = 20;
const OP_RETURN_CALL_REF = 21;
const OP_DELEGATE = 24;
const OP_CATCH_ALL = 25;
const OP_TRY_TABLE = 31;
const OP_REF_EQ = 211;
const OP_REF_AS_NON_NULL = 212;
const OP_BR_ON_NULL = 213;
const OP_BR_ON_NON_NULL = 214;
const OP_GC = 251;
const GC_ONE_ARG = [0, 1, 6, 7, 11, 12, 13, 14, 16, 20, 21, 22, 23];
const GC_TWO_ARGS = [2, 3, 4, 5, 8, 9, 10, 17, 18, 19];
const OP_BULK_MEMORY = 252;
const OP_SIMD = 253;
const OP_ATOMIC = 254;
...
class WailParser extends BufferReader {
  ...
  addTypeEntry(options) {
    ...
    if (typeof returnType === "number") {
      ...
      newEntry.returnType = returnType.map((t) =>
        typeof t === "number" ? t : convertValueType(t),
      );
    }
  ...
  addImportEntry(options) {
    ...
    if (this._importFuncNewCount > 0) {
      ...
      this._optionalSectionFlags |= 1 << SECTION_TABLE;
    }
  ...
  getFunctionIndex(oldIndex) {
    ...
    const pendingOptions = { oldIndex: oldIndex, variable: newVariable };
  ...
  _readSection() {
    ...
    if (id != SECTION_CUSTOM) {
      for (const missingId of SECTION_ORDER.slice(0, rank)) {
  ...
  _readBlockType(reader) {
    const type = reader.inBuffer[reader.inPos];
    if (type == 0x63 || type == 0x64) {
      this._readValType(reader);
    } else {
      reader.readVarUint32();
    }
  }
  _readSubTypes(reader, form) {
    if (form == 0x4e) {
      const count = reader.readVarUint32();
      for (let i = 0; i < count; i++) {
        this._readSubTypes(reader, reader.readUint8());
      }
      return count;
    }
    if (form == 0x50 || form == 0x4f) {
      for (let n = reader.readVarUint32(); n > 0; n--) {
        reader.readVarUint32();
      }
      return this._readSubTypes(reader, reader.readUint8());
    }
    if (form == VALUE_TYPE_FUNC) {
      for (let k = 0; k < 2; k++) {
        for (let n = reader.readVarUint32(); n > 0; n--) {
          this._readValType(reader);
        }
      }
    } else if (form == 0x5f || form == 0x5e) {
      for (let n = form == 0x5f ? reader.readVarUint32() : 1; n > 0; n--) {
        this._readValType(reader);
        reader.readUint8();
      }
    } else {
      throw new Error("Unknown type form " + form);
    }
    return 1;
  }
  _parseTableSection() {
    this.commitBytes();
    const reader = new BufferReader(this.readBytes(this.readVarUint32()));
    for (let n = reader.readVarUint32(); n > 0; n--) {
      const hasInit = reader.inBuffer[reader.inPos] == 0x40;
      if (hasInit) {
        reader.readBytes(2);
      }
      this._readValType(reader);
      ...
      if (hasInit) {
        this._readExpression(reader);
      }
    }
    reader.commitBytes();
    const newPayload = reader.write();
    this.copyBuffer(VarUint32ToArray(newPayload.length));
    this.copyBuffer(newPayload);
  }
  ...
  _addTypeSection() {
    ...
    for (let i = 0; i < newEntries.length; i++) {
      ...
      if (optionsEntry.variable instanceof WailVariable) {
        optionsEntry.variable.value = i;
      }
      reader.copyBuffer(Uint8ToArray(form));
      reader.copyBuffer(VarUint32ToArray(params.length));
      reader.copyBuffer(params);
      if (returnType instanceof Array) {
        reader.copyBuffer(VarUint32ToArray(returnType.length));
        reader.copyBuffer(returnType);
  ...
  _parseTypeSection() {
    ...
    let typeIndex = 0;
    for (let entryIndex = 0; entryIndex < oldCount; entryIndex++) {
      reader.commitBytes();
      const form = reader.readUint8();
      const edits = existingEntries.filter((e) => e.index == typeIndex);
      if (form != VALUE_TYPE_FUNC || edits.length == 0) {
        typeIndex += this._readSubTypes(reader, form);
        continue;
      }
      const thisEntry = edits[edits.length - 1];
      const readVector = () => {
        const at = reader.inPos;
        for (let n = reader.readVarUint32(); n > 0; n--) {
          this._readValType(reader);
        }
        return reader.inBuffer.slice(at, reader.inPos);
      };
      readVector();
      const oldResults = readVector();
      reader.updateCopyPosition();
      const params = thisEntry.params;
      reader.copyBuffer(Uint8ToArray(form));
      reader.copyBuffer(VarUint32ToArray(params.length));
      reader.copyBuffer(params);
      if (typeof thisEntry.returnType !== "undefined") {
        reader.copyBuffer([1, thisEntry.returnType]);
      } else {
        reader.copyBuffer(oldResults);
      }
      typeIndex++;
    }
    reader.commitBytes();
    for (let i = 0; i < newEntries.length; i++) {
      ...
      if (optionsEntry.variable instanceof WailVariable) {
        optionsEntry.variable.value = typeIndex + i;
      }
      reader.copyBuffer(Uint8ToArray(form));
      reader.copyBuffer(VarUint32ToArray(params.length));
      reader.copyBuffer(params);
  ...
  _addCodeSection() {
    ...
    const entryCountArray = VarUint32ToArray(newEntries.length);
    reader.copyBuffer(entryCountArray);
    this._writeNewCodeEntries(reader, 0, newEntries.length);
    const newPayload = reader.write();
    const newPayloadLen = VarUint32ToArray(newPayload.length);
  ...
  _writeNewCodeEntries(reader, oldCount, newCount) {
    const newEntries = this._sectionOptions[SECTION_CODE].newEntries;
  ...
  _parseCodeSection() {
    ...
    this._writeNewCodeEntries(reader, oldCount, newCount);
    const newPayload = reader.write();
    const newPayloadLen = VarUint32ToArray(newPayload.length);
    this.copyBuffer(newPayloadLen);
    this.copyBuffer(newPayload);
  ...
  _readFunction(reader, funcIndex) {
    ...
    for (let i = 0; i < localCount; i++) {
      ...
      this._readValType(headerReader);
    }
  ...
  _readInstruction(reader) {
    ...
    switch (opcode) {
      ...
      case OP_CATCH_ALL:
      case OP_REF_EQ:
      case OP_REF_AS_NON_NULL:
        break;
      case OP_BLOCK:
      case OP_LOOP:
      case OP_IF:
      case OP_TRY:
        this._readBlockType(reader);
        break;
      case OP_TRY_TABLE:
        this._readBlockType(reader);
        for (let n = reader.readVarUint32(); n > 0; n--) {
      ...
      case OP_I64_STORE16:
      case OP_I64_STORE32:
        if (reader.readVarUint32() & 64) reader.readVarUint32();
        reader.readVarUint32();
        break;
      case OP_BR_TABLE:
        const count = reader.readVarUint32();
      ...
      case OP_GC:
        ...
        if (arg == 24 || arg == 25) {
          ...
          reader.readVarUint32();
          reader.readVarUint32();
        } else if (GC_TWO_ARGS.includes(arg)) {
          reader.readVarUint32();
          reader.readVarUint32();
        } else if (GC_ONE_ARG.includes(arg)) {
          reader.readVarUint32();
        } else if (arg != 15 && (arg < 26 || arg > 30)) {
          throw new Error(
            "Unknown argument '" + arg + "' for OP_GC. Probably parsing incorrectly",
          );
        }
        break;
      case OP_REF_IS_NULL:
    ...
    if (typeof this._instructionCallbacks[opcode] !== "undefined") {
      ...
      const callbackResults =
        this._globalInstructionCallback(instrBytes);
      reader.writeAtAnchor(callbackResults);
    }
